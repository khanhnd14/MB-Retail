...
const ChangePinScreen = () => {
  ...
  const changePin = () => {
    ...
    if (confirmPin === oldPin) {
      Utils.toast(I18n.t('softtoken.pin_not_same'))
      return
    }
    setLoading(true)
    const oldPinEncrypt = settingSelectors.md5(oldPin + activeCode)
    dispatch(
      settingOperations.setupPinCode({
        activeCode,
        pinCode: newPin,
        oldPinCode: oldPinEncrypt,
      })
    )
  }
  return (
    <Fragment>
      <Topbar
        title={I18n.t('setting.change_pin_title')}
        isBottomSubLayout
