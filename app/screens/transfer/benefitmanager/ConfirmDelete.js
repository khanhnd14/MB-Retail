...
const ConfirmDelete = (props) => {
  const { visible, handleModal, onConfirm, onModalHide, data } = props
  ...
  return (
    ...
    >
      <View style={styles.container}>
        ...
        <Text style={[styles.fullname, { paddingHorizontal: Metrics.medium * 1.5 }]}>
          {I18n.t('transfer.delete_info')}
        </Text>
        <Text style={styles.fullname}>{data ? data.beneficiaryAlias : ''}</Text>
        <View style={styles.line} />
        <Text style={[styles.fullname, { color: '#4F4F4F' }]}>
          {data ? data.beneficiaryAccountNo : ''}
        </Text>
        <View style={styles.line} />
