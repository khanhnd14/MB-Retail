...
const SelectFDCollatComponent = ({data,onSubmit}) => {
  ...
  const onChoiceCheckbox = (name,ischecked) => {
    ...
    let totalTSBD = 0
    let odLimit = 0;
    let tyLeDamBao = data.tyLeDamBao
    let tyLeDamBaoFSRN = data.tyLeDamBaoFSRN
    let odLimitAmountOnline = data.odLimitAmountOnline
    let odLimitAmountMax = data.odLimitAmountMax
    fdList.forEach(fd => {
            ...
            {data?.FDList?.map((fd, index) => {
              return (
                <TouchableOpacity key={fd.receiptNo} style={styles.fdRowView} onPress={() => onChoiceCheckbox(fd.receiptNo,!fd.isSelected)}>
                  <CheckBox style={styles.fdCheckbox} color={Colors.primary2} 
